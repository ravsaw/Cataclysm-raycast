...
public:
    SDL_Surface* get_image(std::string file) {
        ...
        if (i == cache_.end()) {
            ...
            if (!surf) {
                surf = IMG_Load("./gfx/3d/dummy.png");
...
double posX = 22, posY = 12, posZ = 0;  //x and y start position
double oposX = 22, oposY = 12, oposZ = 0;
double dirX = 0, dirY = -1, dirZ = 0; //initial direction vector
double planeX = 0.7, planeY = 0, planeZ = 0.7; //the 2d raycaster version of camera plane
int shouldDraw = 1;
int scale = 300;
int distancedraw = 20;
image_cache terrianSurface;
SDL_Surface_Ptr buffor;
image_cache skybox;

int raycast = 1;

void cata_tiles::draw(const point & dest, const tripoint & center, int widtho, int heighto,
    ...
    color_block_overlay_container & color_blocks)
{
    ...
    int width = std::min(scale, widtho);
    int height = std::min(scale, heighto);
    ...
    if (state[SDL_SCANCODE_F9]) {
        double oldDirX = dirX;
        dirX = dirX * cos(0.785398) - dirY * sin(0.785398);
        dirY = oldDirX * sin(0.785398) + dirY * cos(0.785398);
        double oldPlaneX = planeX;
        planeX = planeX * cos(0.785398) - planeY * sin(0.785398);
        planeY = oldPlaneX * sin(0.785398) + planeY * cos(0.785398);
        shouldDraw = 1;
    }
    ...
    if( posX != oposX || posY != oposY || posZ != oposZ ) {
        shouldDraw = 1;
        oposX = posX, oposY = posY, oposZ = posZ;
    }

    if (shouldDraw == 0 && raycast == 1) {
        return;
    }

    {
        SDL_Rect clipRect = { dest.x, dest.y, widtho, heighto };
        printErrorIf(SDL_RenderSetClipRect(renderer.get(), &clipRect) != 0, "SDL_RenderSetClipRect failed");
    ...
    if(raycast == 0) {
        ...
        buffor = create_surface_32(width, height);
        SDL_Surface* sky = skybox.get_image("./gfx/3d/skybox.png");
        SDL_BlitScaled(sky, NULL, buffor.get(), NULL);

        for (int x = 0; x < width; x++) {

            for (int y = 0; y < height; y++) {

                double cameraX = 2 * x / static_cast<double>(width) - 1;
                double cameraY = 2 * y / static_cast<double>(height) - 1;

                double rayDirX = dirX + planeX * cameraX;
                double rayDirY = dirY + planeY * cameraX;
                double rayDirZ = dirZ + planeZ * cameraY;
                ...
                double deltaDistZ = (rayDirZ == 0) ? 1e30 : std::abs(1 / rayDirZ);
                ...
                calculateStepAndSideDist(rayDirX, stepX, posX, sideDistX, mapX, deltaDistX);
                calculateStepAndSideDist(rayDirY, stepY, posY, sideDistY, mapY, deltaDistY);
                calculateStepAndSideDist(rayDirZ, stepZ, posZ, sideDistZ, mapZ, deltaDistZ);

                map& here = get_map();
                int hit = 0;
                tripoint terpoint;

                std::string ter_name = "null";
                std::string ter_looks_like_name = "null";


                int zlevel = 0;
                for (int i = 0; i < distancedraw && zlevel < 5; i++)
                {
                    ...
                    tripoint point = tripoint(mapX, mapY, mapZ);
                    const ter_t& curr_ter = here.ter(point).obj();

                    tripoint abovePoint = tripoint(mapX, mapY, mapZ + 1);
                    const ter_t& above_ter = here.ter(abovePoint).obj();

                    if (side == 2 && !above_ter.has_flag(ter_furn_flag::TFLAG_NO_FLOOR) )
                    {
                        hit = 1;
                        ter_looks_like_name = above_ter.looks_like;
                        ter_name = ter_looks_like_name.empty() ? above_ter.id.str() : ter_looks_like_name;
                        break;
                    }
                    else if (side == 3 && !curr_ter.has_flag(ter_furn_flag::TFLAG_NO_FLOOR))
                    {
                        hit = 1;
                        ter_looks_like_name = above_ter.looks_like;
                        ter_name = ter_looks_like_name.empty() ? above_ter.id.str() : ter_looks_like_name;
                    ...
                    else if (!curr_ter.has_flag(ter_furn_flag::TFLAG_TRANSPARENT))
                    {
                        hit = 1;
                        ter_looks_like_name = above_ter.looks_like;
                        ter_name = ter_looks_like_name.empty() ? above_ter.id.str() : ter_looks_like_name;
                ...
                else if (side == 1)     perpWallDist = (sideDistY - deltaDistY);
                else                    perpWallDist = (sideDistZ - deltaDistZ);

                double wallX; //where exactly the wall was hit
                if (side == 0)          wallX = posY + perpWallDist * rayDirY;
                else if (side == 1)     wallX = posX + perpWallDist * rayDirX;
                else                    wallX = posX + perpWallDist * rayDirX;
                wallX -= floor((wallX));

                int texX = int(wallX * static_cast<double>(texWidth));
                ...
                if (side == 0)
                    ...
                    wallY = posY + perpWallDist * rayDirY;
                wallY -= floor((wallY));

                int texY = int(wallY * static_cast<double>(texHeight));

                if (side == 0 && rayDirZ > 0)
                    texY = texWidth - texY - 1;
                if (side == 1 && rayDirZ < 0)
                    texY = texWidth - texY - 1;
                if (side == 2 && rayDirY > 0)
                    texY = texWidth - texY - 1;
                ...
                std::string floororroof = "";

                if (side > 1) 
                    floororroof = side == 2 ? "floor/" : "roof/";

                SDL_Surface* surf = terrianSurface.get_image("./gfx/3d/" + floororroof + ter_name + ".png");

                SDL_Color color = GetPixel(surf, texX, texY);

                if (buffor) {
                    SetPixel(buffor.get(), x, y, color);
                }
            }
        }

        SDL_Rect rect = { dest.x, dest.y, widtho, heighto };
        SDL_Texture_Ptr texture = CreateTextureFromSurface(renderer, buffor);
