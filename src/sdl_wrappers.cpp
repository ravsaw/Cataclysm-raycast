...
void SetPixel(SDL_Surface *surface, int x, int y, SDL_Color pixel)
{
  
  Uint32 * target_pixel = reinterpret_cast<Uint32 *> (static_cast<Uint8 *> (surface->pixels)
                                    + y * surface->pitch + x * surface->format->BytesPerPixel);
  *target_pixel = ColourToUint(pixel.r, pixel.g, pixel.b);
}
...
SDL_Color GetPixel(SDL_Surface *surface, int x, int y)
{
  Uint32 * const target_pixel = reinterpret_cast<Uint32 *>  (static_cast<Uint8 *> (surface->pixels)
                                    + y * surface->pitch + x * surface->format->BytesPerPixel);
  return UintToColour(*target_pixel);
}

Uint32 ColourToUint(int R, int G, int B)
{
    return static_cast<Uint32>((255 << 24) + (R << 16) + (G << 8) + (B << 0));
}
...
void calculateStepAndSideDist(double rayDir, int& step, double pos, double& sideDist, int map, double deltaDist)
{
    if (rayDir < 0)
    {
        step = -1;
